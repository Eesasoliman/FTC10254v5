...
public class NEWBACKBLUE extends DriveOpMode {
    int imageNum;
    ...
    public void runOpMode() {
        ...
        TrajectorySequence purple1 = drive.trajectorySequenceBuilder(startPose)
                ...
                .lineToSplineHeading(new Pose2d(37,28, Math.toRadians(0)))
                .build();
        ...
        TrajectorySequence yellow1 = drive.trajectorySequenceBuilder(new Pose2d(37, 28, Math.toRadians(0)))
                .lineToConstantHeading(new Vector2d(backboardY-5,41.5))
                .lineToSplineHeading(
                        new Pose2d(backboardY+10, 40, Math.toRadians(0)),
                        SampleMecanumDrive.getVelocityConstraint(backVel, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();

        TrajectorySequence yellow2 = drive.trajectorySequenceBuilder(new Pose2d(14, 34.5, Math.toRadians(270)))
                .forward(6)
        ...
        TrajectorySequence yellow3 = drive.trajectorySequenceBuilder(new Pose2d(14.5, 34, Math.toRadians(0)))
                .lineToSplineHeading( new Pose2d(backboardY-5, 29, Math.toRadians(0)))
                .lineToSplineHeading(
                        new Pose2d(backboardY+10, 29, Math.toRadians(0)),
                        SampleMecanumDrive.getVelocityConstraint(backVel, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                        SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                .build();
        ...
        else if (imageNum == 2 || imageNum == 3 || imageNum == 4)
        {
            ...
            if(driveVariables[0])
            {
                if (!driveVariables[2]) {
                    ...
                    drive.followTrajectorySequence(
                            drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                                    .back(5,SampleMecanumDrive.getVelocityConstraint(DriveConstants.MAX_VEL, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                                            SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                                    .build());
//                    prepareScoring(liftHeight);
